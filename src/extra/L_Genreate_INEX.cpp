...
int main(int argc, char **argv)
{
	if(argc != 4)
	{
		cout << "./LBCalib   Dir_path_D2U   Dir_path_U2D InitParampath (eg. ./LBCalib ../testdata/D2U_ALL_1616X1232/ ../testdata/U2D_ALL_1616X1232/ ../testdata/InitParam.txt)" << endl;
		exit(0);
	}
	...
	for (int i = 0; i < nCams; i++)
	{
		...
		for(int y = 0; y < nHight; y++)
		{
			...
			for(int x = 0; x< nWidth; x++)
			{
				...
				std::pair<double, double> tt(v2, v1);
				t.push_back(tt);
			}
